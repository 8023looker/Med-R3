...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        if max_num_batched_tokens < max_model_len and self.config.enable_chunked_prefill:
            raise ValueError(f'Enable chunked prefill, max_num_batched_tokens({max_num_batched_tokens}) is smaller than \
                             max_model_len({max_model_len}), please increase max_num_batched_tokens or disable chunked prefill')
...
class vLLMRolloutWithTool(vLLMRollout):
    ...
    def batch_execute(self, env_list: List[str], tool_calls_list: List[List[str]]):
        ...
        # 展平所有的tool calls和对应的env
        all_tasks = []
        task_indices = []  # 记录每个任务属于哪个env和tool_call
        ...
        # 并行执行所有tasks
        all_results = [None] * len(all_tasks)  # 预分配结果数组
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:        
        ...
        with self.update_sampling_params(**kwargs):
            ...
            for step in range(self.config.max_turns):
                ...
                #     # 从后往前遍历，找到第一个非 eos 和非 pad 的位置
                #     last_valid_idx = -1
                #     for valid_idx in range(len(output_ids) - 1, -1, -1):
                #         if (output_ids[valid_idx] != self.tokenizer.eos_token_id and 
                #             output_ids[valid_idx] != self.tokenizer.pad_token_id):
                #             last_valid_idx = valid_idx
                #             break
                    
                #     if finish_reason == 'stop' and (stop_reason == None or stop_reason == self.tokenizer.pad_token_id):
                #         output_ids = output_ids[:last_valid_idx+1] + [self.tokenizer.eos_token_id]
                ...
                #         curr_inputs[idx] += output_ids
                #         result_mask_list[idx] += [1] * len(output_ids)
                #     elif finish_reason == 'stop' and stop_reason == 151644:
                #         # 从后往前遍历，找到第一个151644的位置
                #         last_imstart_idx = -1
                #         for valid_idx in range(len(output_ids) - 1, -1, -1):
                #             if output_ids[valid_idx] == 151644:
                #                 last_imstart_idx = valid_idx
                #                 break
                #         output_ids = output_ids[:last_imstart_idx+1]
                #         curr_inputs[idx] += output_ids
                #         result_mask_list[idx] += [1] * len(output_ids)
                #     else:
                ...
                for i, idx in enumerate(active_indices):
                    ...
                    if finish_reason == 'stop' and (stop_reason == None or stop_reason == self.tokenizer.pad_token_id):
                        ...
                        curr_inputs[idx] += output_ids
                        result_mask_list[idx] += [1] * len(output_ids)
                    elif finish_reason == 'stop' and stop_reason == 151644: # 151644 is the id of <|im_start|>, is not a illigal stop, we stop here
                        curr_inputs[idx] += output_ids
                        result_mask_list[idx] += [1] * len(output_ids)
                    else:
                        raise ValueError(f"unknown stop reason. finish_reason: {finish_reason}, stop_reason: {stop_reason}")
                ...
                # 针对活跃索引判断是否需要截断，如果需要截断则截断，然后从active中去掉；如果不截断，则更新curr_max_tokens
