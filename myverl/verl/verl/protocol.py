...
class DataProto:
    ...
    def get_acc_info(self) -> Dict:
        """
        get acc information of the prompt group
        """
        if "acc" in self.non_tensor_batch and "uid" in self.non_tensor_batch:
            prompt_uid2metric_vals = defaultdict(list)
            for uid, metric_val in zip(self.non_tensor_batch['uid'], self.non_tensor_batch["acc"]):
                prompt_uid2metric_vals[uid].append(metric_val)
            info = Counter()
            acc_sum = 0
            valid_uid_infos = defaultdict(dict)
            for prompt_uid, metric_vals in prompt_uid2metric_vals.items():
                # print(f"prompt_uid: {prompt_uid}, metric_vals: {metric_vals}")
                info["cnt"] += len(metric_vals)
                info["group_cnt"] += 1
                acc_sum += sum(metric_vals)
                if len(set(metric_vals)) > 1:
                    info['valid_groups'] += 1
                    valid_uid_infos[prompt_uid]['std'] = np.std(metric_vals)
                    valid_uid_infos[prompt_uid]['mean'] = np.mean(metric_vals)
                elif int(list(set(metric_vals))[0]) == 1:
                    info['all_correct_groups'] += 1
                else:
                    info['all_wrong_groups'] += 1
            info['correct_cnt'] = acc_sum
            print(f"Acc Stat Info: {info}")
            info = dict(info)
            info["valid_uid_infos"] = valid_uid_infos
            return info
        return dict()

    def update_acc_info(self, acc_info: Dict):
        old_infos = self.meta_info.get('acc_info', {}).pop("valid_uid_infos", {})
        new_infos = acc_info.pop("valid_uid_infos", {})
        old_infos.update(new_infos)
        self.meta_info['acc_info'] = dict(Counter(self.meta_info.get('acc_info', {})) + Counter(acc_info))
        self.meta_info['acc_info']["valid_uid_infos"] = new_infos

    def acc_report(self):
        info = self.meta_info.get('acc_info', None)
        if info is None:
            return {}
        else:
            try:
                # calculate the std mean
                if "valid_uid_infos" in info:
                    std_mean = np.mean([info["valid_uid_infos"][uid]['std'] for uid in self.non_tensor_batch["uid"] if uid in info["valid_uid_infos"]])
                else:
                    std_mean = 0
                return {
                    "critic/acc/mean(rollout)": info['correct_cnt'] / info['cnt'],
                    "critic/acc/std_mean(rollout)": std_mean,
                    "critic/acc/all_correct_rate": info.get('all_correct_groups', 0) / info['group_cnt'],
                    "critic/acc/all_wrong_rate": info.get('all_wrong_groups', 0) / info['group_cnt'],
                    "critic/acc/group_cnt": info['group_cnt'],
                }
            except Exception as e:
                print("acc_info error", info, e)
                return {}
