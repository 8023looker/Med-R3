...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if max_ckpt_to_keep and isinstance(max_ckpt_to_keep, int) and max_ckpt_to_keep > 0 and len(
            ...
            self.remove_previous_save_local_path(self.previous_saved_paths[:keep_start], only_remove_weights=True)
            self.previous_saved_paths = self.previous_saved_paths[keep_start:]

        local_path = self.local_mkdir(local_path)
        weights_path = self.local_mkdir(os.path.join(local_path, "weights"))
        ...
        with warnings.catch_warnings():
            ...
            with FSDP.state_dict_type(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                model_path = os.path.join(weights_path, f'model_world_size_{self.world_size}_rank_{self.rank}.pt')
                optim_path = os.path.join(weights_path, f'optim_world_size_{self.world_size}_rank_{self.rank}.pt')
                extra_path = os.path.join(weights_path, f'extra_state_world_size_{self.world_size}_rank_{self.rank}.pt')
