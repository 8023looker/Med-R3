...
class BaseCheckpointManager:
    ...
    def remove_previous_save_local_path(self, path, only_remove_weights: bool = False):
        if isinstance(path, str):
            path = [path]
        for p in path:
            abs_path = os.path.abspath(p)
            if only_remove_weights:
                rm_path = os.path.join(abs_path, 'weights')
            else:
                rm_path = abs_path
            print(f'Checkpoint manager remove previous save local path: {rm_path}')
