""" Extract the reasoning KG from the entire reasoning process """
...
class DeepSeekReasoningKG: # extract the reasoning traj
    ...
    def extract_reaoning_KG(self, query_dict, total_template_num=10, temperature=0.9, require_json=True): # for each instance
        ...
        for reasoning_template in reasoning_templates:
            ...
            extract_prompt = prompts._EXTRACT_REASONING_KG_EN.format(
                reasoning_template=tamplate_content,
            )
            # print("extract_prompt: ", extract_prompt)
            # for _ in range(API_MAX_RETRY):
            if_terminate = False
            while not if_terminate:
                try:
                    ...
                    if require_json:
                        extract_resp_content = extract_resp.content.strip("```json`")
                        try:
                            extract_resp_content_loads = json.loads(extract_resp_content)
                            reasoning_trajs.append(extract_resp_content) # {"score": int, "reason": str}
                            if_terminate = True
                        except Exception as e: # ... 会有 KG 解析不出来的情况
                            print(f"########### Error when reading reasoning KG: {e}")
                        # reasoning_trajs.append(extract_resp.content.strip("```json`")) # {"score": int, "reason": str}
        ...
        return {
            "reasoning_KG": reasoning_trajs # List
        }
            ...
            with open(os.path.join(output_dir, os.path.basename(file_path)), "a", encoding="utf-8") as fout:
                for index, item in enumerate(tqdm(jdict, desc=f"Processing {dataset_name}", total=len(jdict))):
                    if index < output_file_line_num: # 断点重启
                        continue
                    response = self.extract_reaoning_KG(query_dict=item, require_json=True)
                    del item["reasoning_templates"]
...
if __name__ == "__main__":
    ...
    api_source = "xxx"
    deepseek_instance = DeepSeekReasoningKG(extract_model, api_source)
    
    raw_data_folder = "/global_data/data/medicalRL/code/data_construction/output_reasoning_embedding/"
    output_root_folder = "/global_data/data/medicalRL/code/data_construction/output_reasoning_KG_v2/"
